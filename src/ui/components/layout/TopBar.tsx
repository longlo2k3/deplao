...
const SUPPORT_GITHUB_URL = "https://github.com/longlo2k3/deplao";
import {
  useAppStore,
  FONT_SCALE_MIN,
  FONT_SCALE_MAX,
...
export default function TopBar() {
  ...
  const { theme, setTheme, showNotification, fontSizeScale, setFontSizeScale } =
    useAppStore();
  ...
  const previewEmployee = previewEmployeeId
    ? employees.find((e: any) => e.employee_id === previewEmployeeId)
    : null;
  ...
  const handleDisconnect = useCallback(async () => {
    ...
    try {
      ...
      const RestQueryService = (
        await import("../../../services/http/RestQueryService")
      ).default;
  ...
  const handleReconnect = useCallback(async () => {
    ...
    if (!bossAddr) {
      setReconnectError("Vui lòng nhập địa chỉ BOSS");
      return;
    }
    if (!username) {
      setReconnectError("Vui lòng nhập tên đăng nhập");
      return;
    }
    if (!password) {
      setReconnectError("Vui lòng nhập mật khẩu");
      return;
    }
    ...
    try {
      ...
      ipc.workspace
        ?.update?.(useWorkspaceStore.getState().activeWorkspaceId, {
          bossUrl: bossAddr,
        } as any)
        .catch(() => {});
      ...
      showNotification(
        `Đã kết nối lại! Xin chào ${employee.display_name}`,
        "success",
      );
  ...
  useEffect(() => {
    if (!ipc.on) return;
    const unsub = ipc.on("erp:event:notification", () =>
      loadUnreadCount(erpEid),
    );
    return () => unsub?.();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("lockScreen:changed", handleChanged);
  }, []);
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (
        !target.closest(".reconnect-popup") &&
        !target.closest(".employee-badge")
      ) {
        setReconnectOpen(false);
      }
    };
  ...
  const handleRequestOldMessages = useCallback(async () => {
    ...
    const activeAccount = useAccountStore
      .getState()
      .accounts.find((a) => a.zalo_id === activeAccountId);
    ...
    try {
      if (isFacebook(activeAccount?.channel)) {
        ...
        const res = await ipc.fb?.getThreads({
        ...
        if (res?.success) {
          ...
          showNotification(
            res?.error || "Không thể đồng bộ hội thoại Facebook",
            "error",
          );
        }
        ...
        const syncRes = await ipc.telegramUser?.refreshMessages({
          accountId: activeAccountId,
        });
        if (!syncRes?.success) {
          showNotification(
            syncRes?.error || "Không thể đồng bộ tin nhắn Telegram",
            "error",
          );
          return;
        }

        const conversationsRes = await DataAccessor.getConversations(
          activeAccountId,
          500,
          0,
        );
        ...
        const insertedText = syncRes.inserted
          ? `, thêm ${syncRes.inserted} tin nhắn`
          : "";
        const pendingText = syncRes.pending
          ? ". Phần còn lại đang tiếp tục tải nền"
          : "";
        showNotification(
          `Đã đồng bộ ${contacts.length} hội thoại${insertedText}${pendingText}`,
          "success",
        );
      } else {
        ...
        if (res?.success) {
          showNotification(
            "Đang tải tin nhắn cũ… Tin nhắn sẽ xuất hiện dần.",
            "success",
          );
        } else {
          ...
          {empMode === "employee" && currentEmployee && (
            <div className="relative flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-gray-800 border border-gray-600 select-none">
                <span
                  className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${bossConnected ? "bg-green-400" : "bg-red-400 animate-pulse"}`}
                />
              ...
              {!bossConnected && (
                <button
                  ...
                  style={{
                    animation: "reconnectShake 0.6s ease-in-out infinite",
                  }}
                >
                  <span className="relative z-10 flex items-center gap-1.5 text-white-important">
                    <span className="drop-shadow-[0_0_4px_rgba(34,197,94,0.6)]">
              ...
              {reconnectOpen && (
                <div className="reconnect-popup absolute left-0 top-full mt-2 w-72 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl z-[9999] p-4">
                  <p className="text-xs text-gray-400 font-medium mb-2">
                    <PluginIcon className="w-4 h-4 inline" /> Kết nối lại với
                    BOSS
                  </p>
                  <div className="space-y-2">
                    ...
                    {savedPassword && (
                      <p className="text-[10px] text-gray-400 text-right">
                        <KeyIcon className="w-4 h-4 inline" /> Đã lưu mật khẩu
                      </p>
                    )}
                    {reconnectError && (
                      <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-2 py-1">
                        <AlertIcon className="w-3 h-3 inline text-red-400" />{" "}
                        {reconnectError}
                      </p>
                    )}
                    <button
          ...
          {empMode !== "employee" && previewEmployee && (
            <div className="flex items-center gap-1.5 ml-2 px-2 py-0.5 rounded-full bg-amber-900/40 border border-amber-600/40">
              <span className="text-[11px] text-amber-300">
                👁 Đang xem: {previewEmployee.display_name}
              </span>
            </div>
          )}
        </div>
              ...
              <div className="absolute right-0 top-full mt-1 w-64 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl z-[9999] overflow-hidden">
                      ...
                      () =>
                        window.dispatchEvent(
                          new CustomEvent("nav:settings", {
                            detail: { tab: "introduction", subtab: "overview" },
                          }),
                        ),
                      80,
                    );
                  ...
                  onClick={() => {
                    ...
                    setTimeout(
                      () =>
                        window.dispatchEvent(
                          new CustomEvent("nav:settings", {
                            detail: { tab: "introduction", subtab: "donate" },
                          }),
                        ),
                      80,
                    );
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400">
                      Các bạn thấy hữu ích có thể ủng hộ dự án nhé!
                    </p>
                  </div>
                </button>
              </div>
